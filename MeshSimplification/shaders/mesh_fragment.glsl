...
void main() {
	...
	for (int i = 0; i < min(point_lights_size, point_lights.length()); ++i) {
		...
		const float attenuation_inv = dot(point_light.attenuation, vec3(1.0, light_distance, pow(light_distance, 2.0)));
		const float attenuation = 1.0 / max(attenuation_inv, 1.0);
		light_direction = normalize(light_direction);

		const float diffuse_intensity = max(dot(light_direction, vertex_normal), 0.0);
		...
		const vec3 view_direction = normalize(-vertex.position.xyz);
		const float specular_intensity = pow(max(dot(reflect_direction, view_direction), 0.0), material.shininess);
