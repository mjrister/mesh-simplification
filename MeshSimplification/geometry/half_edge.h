...
public:
	...
	[[nodiscard]] std::shared_ptr<Vertex> vertex() const { return utils::Get(vertex_); }

	/** \brief Gets the next half-edge of a triangle in counter-clockwise order. */
	[[nodiscard]] std::shared_ptr<HalfEdge> next() const { return utils::Get(next_); }
	...
	[[nodiscard]] std::shared_ptr<HalfEdge> flip() const { return utils::Get(flip_); }
	...
	[[nodiscard]] std::shared_ptr<Face> face() const { return utils::Get(face_); }
