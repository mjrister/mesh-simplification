...
void Scene::Render(ShaderProgram& shader_program, const float delta_time) {
	...
	for (const auto& [mesh, material] : scene_objects_) {
		...
		// in addition to uniform scaling (which is undone when the transformed normal is renormalized)
