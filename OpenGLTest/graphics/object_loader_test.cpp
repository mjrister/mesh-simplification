...
namespace {
	...
	void AssertLoadMeshThrowsWithInvalidArgument(const std::string_view mesh_source) {
		std::istringstream ss{mesh_source.data()};
		ASSERT_THROW(gfx::obj_loader::LoadMesh(ss), std::invalid_argument);
	}
	...
	TEST(ObjLoaderTest, TestLoadMeshWithPositions) {
		...
		const auto mesh = gfx::obj_loader::LoadMesh(ss);
