...
namespace geometry {
	...
	class MeshSimplifier {
		...
		static glm::mat4 ComputeQuadric(const Vertex& v0) {
			glm::mat4 quadric{0.f};

			for (auto edgeij = v0.Edge()->Next()->Flip(); edgeij != v0.Edge(); edgeij = edgeij->Next()->Flip()) {
				const auto& position = v0.Position();
				const auto& normal = edgeij->Face()->Normal();
		...
		static std::pair<std::shared_ptr<Vertex>, GLfloat> GetEdgeContractionVertex(
			HalfEdgeMesh& mesh,
			const std::shared_ptr<const HalfEdge>& edge01,
			...
			if (std::abs(glm::determinant(Q)) < epsilon || std::abs(d) < epsilon) {
				const auto v_new = Vertex::Average(mesh.NextVertexId(), *v0, *v1);
			...
			return { std::make_shared<Vertex>(mesh.NextVertexId(), position, normal), cost };
		}

		void Simplify(const HalfEdgeMesh& mesh, const GLfloat stop_ratio) {
			...
			std::priority_queue<std::shared_ptr<HalfEdge>> edge_queue;
			...
			while (!edge_queue.empty() && face_count * stop_ratio > mesh.Faces().size()) {
				...
				if (valid_edges.count(hash_value(*entry))) {
